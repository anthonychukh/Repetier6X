...
void PrintLine::moveRelativeDistanceInSteps(int32_t x, int32_t y, int32_t z, int32_t e, float feedrate, bool waitEnd, bool checkEndstop, bool pathOptimize) {
    ...
    Com::printFLN(PSTR("moveRelativeDistanceInSteps 3axis"));
#endif
#if NUM_EXTRUDER > 0
...
//6axis
void PrintLine::moveRelativeDistanceInSteps(int32_t x, int32_t y, int32_t z, int32_t a, int32_t b, int32_t c, int32_t e, float feedrate, bool waitEnd, bool checkEndstop, bool pathOptimize) {
#ifdef DEBUG_STEPCOUNT
    Com::printFLN(PSTR("moveRelativeDistanceInSteps 6axis"));
#endif
    ...
    if(!Printer::isHoming() && !Printer::isNoDestinationCheck()) {
        ...
        if(!Printer::isZHomed() && !Printer::isZProbingActive())
            z = 0;
#endif
#if MOVE_A_WHEN_HOMED && NUM_AXES >=4
        if(!Printer::isAHomed())
            a = 0;
#endif
#if MOVE_B_WHEN_HOMED && NUM_AXES >=5
        if(!Printer::isBHomed())
            b = 0;
#endif
#if MOVE_C_WHEN_HOMED && NUM_AXES >=6
        if(!Printer::isCHomed())
            c = 0;
#endif
    }
    ...
    Printer::destinationSteps[B_AXIS] = Printer::currentPositionSteps[B_AXIS] + b;
    Printer::destinationSteps[C_AXIS] = Printer::currentPositionSteps[C_AXIS] + c;
    Printer::destinationSteps[E_AXIS] = Printer::currentPositionSteps[E_AXIS] + e;
    Printer::feedrate = feedrate;
#if NONLINEAR_SYSTEM
    if (!queueNonlinearMove(checkEndstop, pathOptimize, false)) {
...
void PrintLine::moveRelativeDistanceInStepsReal(int32_t x, int32_t y, int32_t z, int32_t e, float feedrate, bool waitEnd, bool pathOptimize) {
    ...
    if(!Printer::isPositionAllowed( Printer::lastCmdPos[X_AXIS], Printer::lastCmdPos[Y_AXIS], Printer::lastCmdPos[Z_AXIS], Printer::lastCmdPos[A_AXIS], Printer::lastCmdPos[B_AXIS], Printer::lastCmdPos[C_AXIS])) {  //6axis isPositionAllow
        return; // ignore move
    }
#if NUM_EXTRUDER > 0
    if(Printer::debugDryrun() || (MIN_EXTRUDER_TEMP > 30 && Extruder::current->tempControl.currentTemperatureC < MIN_EXTRUDER_TEMP && !Printer::isColdExtrusionAllowed() && Extruder::current->tempControl.sensorType != 0))
...
//6axis
void PrintLine::moveRelativeDistanceInStepsReal(int32_t x, int32_t y, int32_t z, int32_t a, int32_t b, int32_t c, int32_t e, float feedrate, bool waitEnd, bool pathOptimize) {
#if MOVE_X_WHEN_HOMED == 1 || MOVE_Y_WHEN_HOMED == 1 || MOVE_Z_WHEN_HOMED == 1
    if(!Printer::isHoming() && !Printer::isNoDestinationCheck()) { // prevent movements when not homed
#if MOVE_X_WHEN_HOMED
        if(!Printer::isXHomed())
    ...
    Printer::lastCmdPos[B_AXIS] += b * Printer::invAxisStepsPerMM[B_AXIS];
    Printer::lastCmdPos[C_AXIS] += c * Printer::invAxisStepsPerMM[C_AXIS];
#if LAZY_DUAL_X_AXIS
    Printer::sledParked = false;
#endif
    ...
    // Printer::moveToReal(Printer::lastCmdPos[X_AXIS], Printer::lastCmdPos[Y_AXIS], Printer::lastCmdPos[Z_AXIS],
    //                     (Printer::currentPositionSteps[E_AXIS] + e) * Printer::invAxisStepsPerMM[E_AXIS], feedrate, pathOptimize);
    //6axis   replace with 6axis version
    Printer::moveToReal(Printer::lastCmdPos[X_AXIS], Printer::lastCmdPos[Y_AXIS], Printer::lastCmdPos[Z_AXIS],
                         Printer::lastCmdPos[A_AXIS], Printer::lastCmdPos[B_AXIS], Printer::lastCmdPos[C_AXIS],
                        (Printer::currentPositionSteps[E_AXIS] + e) * Printer::invAxisStepsPerMM[E_AXIS], feedrate, pathOptimize);
...
void PrintLine::calculateMove(float axisDistanceMM[], uint8_t pathOptimize, fast8_t drivingAxis) {
...
#if NUM_AXES >=4
    if(isAMove()) {
        axisInterval[A_AXIS] = axisDistanceMM[A_AXIS] * toTicks / (Printer::maxFeedrate[A_AXIS]); // mm*ticks/s/(mm/s*steps) = ticks/step
#if !NONLINEAR_SYSTEM || defined(FAST_COREXYZ)
...
#if NUM_AXES >=5
    if(isBMove()) {
        axisInterval[B_AXIS] = axisDistanceMM[B_AXIS] * toTicks / (Printer::maxFeedrate[B_AXIS]); // mm*ticks/s/(mm/s*steps) = ticks/step
#if !NONLINEAR_SYSTEM || defined(FAST_COREXYZ)
        limitInterval = RMath::max(axisInterval[B_AXIS], limitInterval);
#endif
    } else axisInterval[B_AXIS] = 0;
#endif

#if NUM_AXES >=6
    if(isCMove()) {
        axisInterval[C_AXIS] = axisDistanceMM[C_AXIS] * toTicks / (Printer::maxFeedrate[C_AXIS]); // mm*ticks/s/(mm/s*steps) = ticks/step
#if !NONLINEAR_SYSTEM || defined(FAST_COREXYZ)
        limitInterval = RMath::max(axisInterval[C_AXIS], limitInterval);
#endif
    } else axisInterval[C_AXIS] = 0;
#endif
    ...
    if(isAMove()) {
        ...
        speedA = axisDistanceMM[A_AXIS] * inverseTimeS;
        if(isANegativeMove()) speedA = -speedA;
    } else speedA = 0;
    if(isBMove()) {
        axisInterval[B_AXIS] = timeForMove / delta[B_AXIS];
        speedB = axisDistanceMM[B_AXIS] * inverseTimeS;
        if(isBNegativeMove()) speedB = -speedB;
    } else speedB = 0;
    if(isCMove()) {
        axisInterval[C_AXIS] = timeForMove / delta[C_AXIS];
        speedC = axisDistanceMM[C_AXIS] * inverseTimeS;
        if(isCNegativeMove()) speedC = -speedC;
    } else speedC = 0;
...
/**
  ...
  nested inside PrintLine::calculateNonlinearSubSegments. Check for illegal move
*/
...
#ifdef DEBUG_STEPCOUNT
        Com::printFLN(PSTR("Is large machine"));//test not large machine
#endif
...
//6axis
        // deltaPosSteps[A_AXIS] = cartesianPosSteps[A_AXIS];
        // deltaPosSteps[B_AXIS] = cartesianPosSteps[B_AXIS];
        // deltaPosSteps[C_AXIS] = cartesianPosSteps[C_AXIS];
        return 1;
#endif
    } else {
#ifdef DEBUG_STEPCOUNT
        Com::printFLN(PSTR("Is NOT large machine"));
#endif
...
bool NonlinearSegment::checkEndstops(PrintLine *cur, bool checkall) {
    ...
    if(Printer::isZProbingActive()) {
        ...
        if(isZNegativeMove() && Endstops::zProbe()) {
            ...
            cur->setAMoveFinished();
            cur->setBMoveFinished();
            cur->setCMoveFinished();
            //dir = 0;
...
void PrintLine::calculateDirectionAndDelta(int32_t difference[], ufast8_t *dir, int32_t delta[]) {
    *dir = 0;
    //Find direction
    if(difference[X_AXIS] != 0) {  //changed 
...
// #if NUM_AXES >= 4
//     if(difference[A_AXIS] != 0) {  //changed 
//         if(difference[A_AXIS] < 0) {  
//             delta[A_AXIS] = -difference[A_AXIS];
//             *dir |= XSTEP;
//         } else {
//             delta[A_AXIS] = difference[A_AXIS];
//             *dir |= A_DIRPOS + ASTEP;
...
//     if(difference[B_AXIS] != 0) {  //changed 
//         if(difference[B_AXIS] < 0) {  
//             delta[B_AXIS] = -difference[B_AXIS];
//             *dir |= XSTEP;
//         } else {
//             delta[B_AXIS] = difference[B_AXIS];
//             *dir |= B_DIRPOS + BSTEP;
...
//     if(difference[C_AXIS] != 0) {  //changed 
//         if(difference[C_AXIS] < 0) {  
//             delta[C_AXIS] = -difference[C_AXIS];
//             *dir |= CSTEP;
//         } else {
//             delta[C_AXIS] = difference[C_AXIS];
//             *dir |= C_DIRPOS + CSTEP;
...
void PrintLine::calculateDirectionAndDelta(int32_t difference[], ufast8_t *dir, ufast8_t *dirABC, int32_t delta[]) {
    *dir = 0;
    *dirABC = 0;
    //Find direction
    if(difference[X_AXIS] != 0) {  //changed 
...
inline uint16_t PrintLine::calculateNonlinearSubSegments(uint8_t softEndstop) {
    fast8_t i;
    int32_t delta, diff;
    int32_t destinationSteps[TOWER_ARRAY], destinationDeltaSteps[TOWER_ARRAY];
    //int32_t destinationSteps[Z_AXIS_ARRAY], destinationDeltaSteps[TOWER_ARRAY];
    //Since Z_AXS_ARRAY now include ABC, ie = 6, use TOWER_ARRAY, which equal to original Z_AXIS_ARRAY
...
#else
    float dx[TOWER_ARRAY]; //changed form Z array to tower array
    for(int i = 0; i < TOWER_ARRAY; i++) //changed form Z array to tower array
...
//  out.println_byte_P(PSTR("Calculate delta segments:"), p->numDeltaSegments);
    ...
    for (int s = numNonlinearSegments; s > 0; s--) {
        ...
        for(i = 0; i < TOWER_ARRAY; i++) // End of segment in Cartesian steps originally Z_AXIS_ARRAY
        ...
        if (transformCartesianStepsToDeltaSteps(destinationSteps, destinationDeltaSteps)) {
            ...
            for(i = 0; i < TOWER_ARRAY; i++) {
                ...
                //write to position step saved in printer
...
#else
/**
  6axis see above for 3 axis version.
  Calculate and cache the delta robot positions of the Cartesian move in a line.
  @return The largest delta axis move in a single segment
  @param p The line to examine.
*/
...
#ifdef DEBUG_STEPCOUNT
    Com::printF(PSTR(" calculateNonlinearSubSegments  BEFORE::"));
    Com::printF(PSTR(" x:"), Printer::currentNonlinearPositionSteps[X_AXIS]);
    Com::printF(PSTR(" y:"), Printer::currentNonlinearPositionSteps[Y_AXIS]);
    Com::printF(PSTR(" z:"), Printer::currentNonlinearPositionSteps[Z_AXIS]);
    Com::printF(PSTR(" a:"), Printer::currentNonlinearPositionSteps[A_AXIS]);
    Com::printF(PSTR(" b:"), Printer::currentNonlinearPositionSteps[B_AXIS]);
    Com::printFLN(PSTR(" c:"), Printer::currentNonlinearPositionSteps[C_AXIS]);
#endif
    fast8_t i;
    int32_t delta, diff;
    //int32_t destinationSteps[TOWER_ARRAY], destinationDeltaSteps[TOWER_ARRAY];
    int32_t destinationSteps[Z_AXIS_ARRAY], destinationDeltaSteps[Z_AXIS_ARRAY]; ///step to move i.e dx
    //Since Z_AXS_ARRAY now include ABC, ie = 6, use TOWER_ARRAY, which equal to original Z_AXIS_ARRAY
    // Save current position
#if (CPU_ARCH == ARCH_AVR) && !EXACT_DELTA_MOVES
    for(uint8_t i = 0; i < Z_AXIS_ARRAY; i++)
        destinationSteps[i] = Printer::currentPositionSteps[i];
#else
    float dx[Z_AXIS_ARRAY]; //changed form Z array to tower array
    for(int i = 0; i < Z_AXIS_ARRAY; i++){ //changed form Z array to tower array
        dx[i] = static_cast<float>(Printer::destinationSteps[i] - Printer::currentPositionSteps[i]) / static_cast<float>(numNonlinearSegments);
#ifdef DEBUG_STEPCOUNT
        Com::printFLN(PSTR("Printer::destinationSteps[i] - Printer::currentPositionSteps[i]:  "), dx[i]);
#endif
    }
#endif
//  out.println_byte_P(PSTR("Calculate delta segments:"), p->numDeltaSegments);
    ...
    for (int s = numNonlinearSegments; s > 0; s--) {
...
#else
        float segment = static_cast<float>(numNonlinearSegments - s + 1);
        for(i = 0; i < Z_AXIS_ARRAY; i++){ // End of segment in Cartesian steps originally Z_AXIS_ARRAY
            // Perfect approximation, but slower, so we limit it to faster processors like arm
            destinationSteps[i] = static_cast<int32_t>(floor(0.5 + dx[i] * segment)) + Printer::currentPositionSteps[i];
#ifdef DEBUG_STEPCOUNT
            Com::printFLN(PSTR("floor(0.5 + dx[i] * segment)) + Printer::currentPositionSteps[i]:  "), destinationSteps[i]);
#endif
        }
#endif
        // Verify that delta calculation has a solution
        if (transformCartesianStepsToDeltaSteps(destinationSteps, destinationDeltaSteps)) {
            ...
            if (softEndstop) {
                ...
                destinationDeltaSteps[B_TOWER] = RMath::min(destinationDeltaSteps[B_TOWER], Printer::maxDeltaPositionSteps);
                destinationDeltaSteps[C_TOWER] = RMath::min(destinationDeltaSteps[C_TOWER], Printer::maxDeltaPositionSteps);
                destinationDeltaSteps[A_AXIS] = destinationSteps[A_AXIS];// + SQRT(Printer::deltaDiagonalStepsSquaredA.l); //RMath::min(destinationSteps[A_AXIS], Printer::maxDeltaPositionSteps);
                destinationDeltaSteps[B_AXIS] = destinationSteps[B_AXIS];// + SQRT(Printer::deltaDiagonalStepsSquaredA.l); //RMath::min(destinationSteps[B_AXIS], Printer::maxDeltaPositionSteps);//
                destinationDeltaSteps[C_AXIS] = destinationSteps[C_AXIS];// + SQRT(Printer::deltaDiagonalStepsSquaredA.l); // RMath::min(destinationSteps[C_AXIS], Printer::maxDeltaPositionSteps);//
            }
            else{ //on home axis softendstop == false
                //6axis this should mean 0 step at setup
                destinationDeltaSteps[A_AXIS] = destinationSteps[A_AXIS];// + SQRT(Printer::deltaDiagonalStepsSquaredA.l); //RMath::min(destinationSteps[A_AXIS], Printer::maxDeltaPositionSteps);
                destinationDeltaSteps[B_AXIS] = destinationSteps[B_AXIS];// + SQRT(Printer::deltaDiagonalStepsSquaredA.l); //RMath::min(destinationSteps[B_AXIS], Printer::maxDeltaPositionSteps);//
                destinationDeltaSteps[C_AXIS] = destinationSteps[C_AXIS];// + SQRT(Printer::deltaDiagonalStepsSquaredA.l); // RMath::min(destinationSteps[C_AXIS], Printer::maxDeltaPositionSteps);//
            }
            ...
            for(i = 0; i < TOWER_ARRAY; i++) {
                delta = destinationDeltaSteps[i] - Printer::currentNonlinearPositionSteps[i];
#ifdef DEBUG_STEPCOUNT
                Com::printFLN(PSTR("destinationDeltaSteps[i] - currentNonlinearPositionSteps[i]:  "), delta);
#endif
                if (delta > 0) {
                    d->setPositiveMoveOfAxis(i);
#ifdef DEBUG_DELTA_OVERFLOW
...
//6axis
            for(i = TOWER_ARRAY; i < Z_AXIS_ARRAY; i++) {
                delta = destinationDeltaSteps[i] - Printer::currentNonlinearPositionSteps[i];
                if (delta > 0) {
#ifdef DEBUG_STEPCOUNT
                Com::printFLN(PSTR("CalNonLinearSeg:: setPositiveMoveOfABCAxis: axis"), i - TOWER_ARRAY);
#endif
                    d->setPositiveMoveOfABCAxis(i - TOWER_ARRAY);
#ifdef DEBUG_DELTA_OVERFLOW
                    if (delta > 65535)
                        Com::printFLN(Com::tDBGDeltaOverflow, delta);
#endif
...
#ifdef DEBUG_STEPCOUNT //6axis 
                Com::printFLN(PSTR("CalNonLinearSeg:: setMoveOfABCAxis: axis"), i - TOWER_ARRAY);
#endif
                    d->setMoveOfABCAxis(i - TOWER_ARRAY);
#ifdef DEBUG_DELTA_OVERFLOWzz
                    if (-delta > 65535)
                        Com::printFLN(Com::tDBGDeltaOverflow, delta);
#endif
            ...
            Com::printF(PSTR(" calculateNonlinearSubSegments::"));
            Com::printF(PSTR(" x:"), Printer::currentNonlinearPositionSteps[X_AXIS]);
            Com::printF(PSTR(" y:"), Printer::currentNonlinearPositionSteps[Y_AXIS]);
            Com::printF(PSTR(" z:"), Printer::currentNonlinearPositionSteps[Z_AXIS]);
            Com::printF(PSTR(" a:"), Printer::currentNonlinearPositionSteps[A_AXIS]);
            ...
            Com::printF(PSTR(" a:"), destinationSteps[A_AXIS]);
            Com::printF(PSTR(" b:"), destinationSteps[B_AXIS]);
            Com::printFLN(PSTR(" c:"), destinationSteps[C_AXIS]);
            d->dir = 0;
            d->deltaSteps[A_TOWER] = d->deltaSteps[B_TOWER] = d->deltaSteps[C_TOWER]  = d->deltaSteps[A_AXIS] = d->deltaSteps[B_AXIS] = d->deltaSteps[C_AXIS] = 0;
            return 65535; // flag error
        }
    }
#ifdef DEBUG_STEPCOUNT
...
uint8_t PrintLine::calculateDistance(float axisDistanceMM[], uint8_t dir, uint8_t dirABC,  float *distance) {
    // Calculate distance depending on direction
    if(dir & XYZ_STEP) {
        if(dir & XSTEP)
            *distance = axisDistanceMM[X_AXIS] * axisDistanceMM[X_AXIS];
    ...
    if(dirABC & ABC_STEP) {
        if(dirABC & ASTEP)
            *distance = axisDistanceMM[A_AXIS] * axisDistanceMM[A_AXIS];
        else
            *distance = 0;
        if(dirABC & BSTEP)
            *distance += axisDistanceMM[B_AXIS] * axisDistanceMM[B_AXIS];
        if(dirABC & CSTEP)
            *distance += axisDistanceMM[C_AXIS] * axisDistanceMM[C_AXIS];
        *distance = RMath::max((float)sqrt(*distance), axisDistanceMM[E_AXIS]);
        //return 1;
    } 

    if(dirABC & ABC_STEP || dir & XYZ_STEP){
        return 1;
    }
    else {
        if(dir & ESTEP) {
...
uint8_t PrintLine::queueNonlinearMove(uint8_t check_endstops, uint8_t pathOptimize, uint8_t softEndstop) {
...
#ifdef DEBUG_STEPCOUNT
    Com::printFLN(PSTR(" queueNonlinearMove "));
#endif
    ...
    for(fast8_t axis = 0; axis < E_AXIS_ARRAY; axis++) {
        difference[axis] = Printer::destinationSteps[axis] - Printer::currentPositionSteps[axis]; //desitination step cal in Printer::setDestinationStepsFromGCode(GCode *com)
        if(axis == E_AXIS) {
            if(Printer::mode == PRINTER_MODE_FFF) {
    ...
    ///float abcDistance; //6axis
    ufast8_t cartesianDir;
    ufast8_t abcDir; //6axis
    int32_t cartesianDeltaSteps[E_AXIS_ARRAY];
#if NUM_AXES <= 3
    calculateDirectionAndDelta(difference, &cartesianDir, cartesianDeltaSteps);
#else
    calculateDirectionAndDelta(difference, &cartesianDir, &abcDir, cartesianDeltaSteps);//6axis
#endif

#ifdef DEBUG_STEPCOUNT
    Com::printF(PSTR(" queueNonlinearMove:: DeltaStepsX : "), cartesianDeltaSteps[X_AXIS]);
    Com::printF(PSTR(" DeltaStepsY : "), cartesianDeltaSteps[Y_AXIS]);
    Com::printF(PSTR(" DeltaStepsZ : "), cartesianDeltaSteps[Z_AXIS]);
    Com::printF(PSTR(" DeltaStepsA : "), cartesianDeltaSteps[A_AXIS]);
    Com::printF(PSTR(" DeltaStepsB : "), cartesianDeltaSteps[B_AXIS]);
    Com::printFLN(PSTR(" DeltaStepsC : "), cartesianDeltaSteps[C_AXIS]);
    
#endif

    if (!calculateDistance(axisDistanceMM, cartesianDir, abcDir, &cartesianDistance)) { //if no distance detected
        // Appears the intent is to do nothing if no distance is detected.
        // This apparently is not an error condition, just early exit.
#ifdef DEBUG_STEPCOUNT
        Com::printFLN(PSTR(" queueNonlinearMove::calculateDistance return true "));
#endif
        return true;
    }
    ...
    if ( !((cartesianDir & XYZ_STEP) || (abcDir & ABC_STEP)) ) {  //if only E move
        queueEMove(difference[E_AXIS], check_endstops, pathOptimize);
#ifdef DEBUG_STEPCOUNT
        Com::printFLN(PSTR(" queueNonlinearMove::only E move "));
#endif
        return true;
    }
...
#ifdef DEBUG_STEPCOUNT
    Com::printFLN(PSTR(" segmentCount = "), segmentCount);
    Com::printFLN(PSTR(" number of line = "), numLines);
#endif
    ...
    for (int lineNumber = 1; lineNumber <= numLines; lineNumber++) {
        waitForXFreeLines(1);
        PrintLine *p = getNextWriteLine(); //pointer to current line
...
// #else
//             calculateDirectionAndDelta(fractionalSteps, &p->dir, &p->dirABC,  p->delta);
// #endif
            p->distance = cartesianDistance;
        }
        ...
        uint16_t maxStepsPerSegment = p->calculateNonlinearSubSegments(softEndstop);//set XYZ stepmove

        if (maxStepsPerSegment == 65535) { //flag error
        ...
        if (virtualAxisSteps == 0 && p->delta[E_AXIS] == 0 && !(abcDir & ABC_STEP)) { //if no XYZ or ABC move, !(abcDir & ABC_STEP) cause segmentation failure
            if (numLines != 1) {
                Com::printErrorFLN(Com::tDBGDeltaNoMoveinDSegment);
                return false;  // Line too short in low precision area
            }
#ifdef DEBUG_STEPCOUNT
            Com::printFLN(PSTR(" virtual axis step == 0"));
#endif
			break;
        }
...
#ifdef DEBUG_STEPCOUNT
        Com::printF(PSTR(" queueNonlinearMove:: curPosX :"), Printer::currentPositionSteps[X_AXIS]);
        Com::printF(PSTR(" curPosY :"), Printer::currentPositionSteps[Y_AXIS]);
        Com::printF(PSTR(" curPosZ :"), Printer::currentPositionSteps[Z_AXIS]);
        Com::printF(PSTR(" curPosA :"), Printer::currentPositionSteps[A_AXIS]);
        Com::printF(PSTR(" curPosB :"), Printer::currentPositionSteps[B_AXIS]);
        Com::printFLN(PSTR(" curPosC :"), Printer::currentPositionSteps[C_AXIS]);
#endif
    }
...
int32_t PrintLine::bresenhamStep() { // Version for delta printer
...
#endif
    {
        ...
        // if(cur->isAMove()) {
        //     Printer::enableAStepper();  //6axis
        // }
        // if(cur->isBMove()) 
        //     Printer::enableBStepper();
        // if(cur->isCMove()) 
        //     Printer::enableCStepper();
//         if(cur->isAMove() || cur->isBMove() || cur->isCMove()){
//             Printer::enableAStepper();
//             Printer::enableBStepper();
//             Printer::enableCStepper();
// #ifdef DEBUG_STEPCOUNT
//             Com::printFLN(PSTR("bresenhamStep:: ABC stepper enable"));
// #endif
//         }
...
#ifdef DEBUG_STEPCOUNT
            Com::printFLN(PSTR("bresenhamStep:: XYZ stepper enable"));
#endif
#if NUM_AXES >=4
            Printer::enableAStepper();  //6axis because abc now in nonlinearSegmen. see PrintLine::calculateNonlinearSubSegments
            Printer::enableBStepper();
            Printer::enableCStepper();
            Printer::setADirection(curd->isAPositiveMove());
            Printer::setBDirection(curd->isBPositiveMove());
            Printer::setCDirection(curd->isCPositiveMove());
#ifdef DEBUG_STEPCOUNT
            Com::printFLN(PSTR("bresenhamStep:: ABC stepper enable"));
#endif
#endif
...
#else
            cur->error[X_AXIS] = cur->error[Y_AXIS] = cur->error[Z_AXIS] = cur->error[A_AXIS] = cur->error[B_AXIS] = cur->error[C_AXIS] = cur->numPrimaryStepPerSegment >> 1;
#endif
        ...
        // Printer::setBDirection(cur->isBPositiveMove());
        // Printer::setCDirection(cur->isCPositiveMove());
...
// #endif
// #ifdef DEBUG_STEPCOUNT
//                 Com::printFLN(PSTR(" isAMove"));
// #endif
//             if((cur->error[A_AXIS] -= cur->delta[A_AXIS]) < 0) {
//                 cur->startAStep();
//                 cur->error[A_AXIS] += cur_errupd;
// #ifdef DEBUG_STEPCOUNT
//                 Com::printFLN(PSTR(" startAStep"));
// #endif
//             }
// #if CPU_ARCH == ARCH_AVR
//         if(cur->isBMove())
// #endif
//             if((cur->error[B_AXIS] -= cur->delta[B_AXIS]) < 0) {
//                 cur->startBStep();
//                 cur->error[B_AXIS] += cur_errupd;
// #ifdef DEBUG_STEPCOUNT
//                 Com::printFLN(PSTR(" startBStep"));
// #endif
//             }
// #if CPU_ARCH == ARCH_AVR
//         if(cur->isCMove())
// #endif
//             if((cur->error[C_AXIS] -= cur->delta[C_AXIS]) < 0) {
//                 cur->startCStep();
//                 cur->error[C_AXIS] += cur_errupd;
//             }
...
//6axis
            ...
            if (curd->isAMove())
                if ((cur->error[A_AXIS] -= curd->deltaSteps[A_AXIS]) < 0)
                {
                    cur->startAStep();
                    cur->error[A_AXIS] += curd_errupd;
                    //Printer::realDeltaPositionSteps[A_AXIS] += curd->isAPositiveMove() ? 1 : -1;
#ifdef DEBUG_STEPCOUNT
                    cur->totalStepsRemaining--;
                    //Com::printFLN(PSTR(" startAStep"));
#endif
            ...
            if (curd->isBMove())
                if ((cur->error[B_AXIS] -= curd->deltaSteps[B_AXIS]) < 0)
                {
                    cur->startBStep();
                    cur->error[B_AXIS] += curd_errupd;
                    //Printer::realDeltaPositionSteps[B_AXIS] += curd->isBPositiveMove() ? 1 : -1;
#ifdef DEBUG_STEPCOUNT
                    cur->totalStepsRemaining--;
                    //Com::printFLN(PSTR(" startBStep"));
#endif
            ...
            if (curd->isCMove())
                if ((cur->error[C_AXIS] -= curd->deltaSteps[C_AXIS]) < 0)
                {
                    cur->startCStep();
                    cur->error[C_AXIS] += curd_errupd;
                    //Printer::realDeltaPositionSteps[C_AXIS] += curd->isCPositiveMove() ? 1 : -1;
#ifdef DEBUG_STEPCOUNT
                    cur->totalStepsRemaining--;
                    //Com::printFLN(PSTR(" startBStep"));
#endif
        ...
        if (!stepsPerSegRemaining) { // start new nonlinear segment
            if (cur->numNonlinearSegments && curd != NULL) {
                ...
                Printer::setADirection(curd->dir & A_DIRPOS);
                Printer::setBDirection(curd->dir & B_DIRPOS);
                Printer::setCDirection(curd->dir & C_DIRPOS);
#ifdef DEBUG_STEPCOUNT
                Com::printFLN(PSTR("bresenhamStep:: loop end"));
#endif
#if defined(DIRECTION_DELAY) && DIRECTION_DELAY > 0
#if CPU_ARCH != ARCH_AVR
                if(loop < maxLoops - 1)
...
// #ifdef DEBUG_STEPCOUNT
//     Com::printFLN(PSTR(" bresenhamStep end"));
// #endif
    return Printer::interval;
