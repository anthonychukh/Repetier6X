...
#define PRINTER_FLAG4_A_HOMED               1  //6axis homing
#define PRINTER_FLAG4_B_HOMED               2
#define PRINTER_FLAG4_C_HOMED               4
...
public:
    ...
    static uint8_t flag2, flag3;  //flag2 for settings(autoretract) flag3 for homming state
    static uint8_t flag4;       //6axis for homing abc
...
#else
    static int32_t currentNonlinearPositionSteps[E_AXIS_ARRAY];
#endif
...
#else
    static int32_t realDeltaPositionSteps[Z_AXIS_ARRAY];
#endif
...
#endif
#if NUM_AXES >= 4
    static int32_t aMinStepsAdj, bMinStepsAdj, cMinStepsAdj; // adjusted to cover extruder/probe offsets
    static int32_t aMaxStepsAdj, bMaxStepsAdj, cMaxStepsAdj;
#endif
    ...
    static int32_t aMaxSteps;                   ///< For software endstops, limit of move in positive direction.
    static int32_t bMaxSteps;                   ///< For software endstops, limit of move in positive direction.
    static int32_t cMaxSteps;                   ///< For software endstops, limit of move in positive direction.
    static int32_t aMinSteps;                   ///< For software endstops, limit of move in negative direction.
    static int32_t bMinSteps;                   ///< For software endstops, limit of move in negative direction.
    static int32_t cMinSteps;                   ///< For software endstops, limit of move in negative direction.
    ...
    static float zMin;
    static float aLength;
    static float aMin;
    static float bLength;
    static float bMin;
    static float cLength;
    static float cMin;
    ...
    static float memoryA;
    static float memoryB;
    static float memoryC;
    ...
    static INLINE void disableZStepper() {
        ...
        static INLINE void disableAStepper() {  //6axis disable stepper
#if (A_ENABLE_PIN > -1 && NUM_AXES>=4)
        WRITE(A_ENABLE_PIN, !A_ENABLE_ON);
#endif
    }
            static INLINE void disableBStepper() {
#if (B_ENABLE_PIN > -1 && NUM_AXES>=5)
        WRITE(B_ENABLE_PIN, !B_ENABLE_ON);
#endif
    }
            static INLINE void disableCStepper() {
    ...
    static INLINE void  enableBStepper() {
#if (B_ENABLE_PIN > -1 && NUM_AXES>=5)
        WRITE(B_ENABLE_PIN, B_ENABLE_ON);
#endif
    }
    static INLINE void  enableCStepper() {
#if (C_ENABLE_PIN > -1 && NUM_AXES>=6)
        WRITE(C_ENABLE_PIN, C_ENABLE_ON);
#endif
    }
    ...
    static INLINE void setBDirection(bool positive)
    {
    #if NUM_AXES>=5
        if (positive)
        {
            WRITE(B_DIR_PIN, !INVERT_B_DIR);
        }
        else
        {
            WRITE(B_DIR_PIN, INVERT_B_DIR);
    ...
    static INLINE void setCDirection(bool positive)
    {
    #if  NUM_AXES>=6
        if (positive)
        {
    ...
    static INLINE bool getBDirection() {
    #if  NUM_AXES>=5
        return ((READ(B_DIR_PIN) != 0) ^ INVERT_B_DIR);
    #endif
    }
    static INLINE bool getCDirection() {
    #if  NUM_AXES>=6
        return ((READ(C_DIR_PIN) != 0) ^ INVERT_C_DIR);
    #endif
    }
    ...
    #if NUM_AXES >=4
        flag4 &= ~(PRINTER_FLAG4_A_HOMED | PRINTER_FLAG4_B_HOMED | PRINTER_FLAG4_C_HOMED);
    #endif
    }

    static INLINE void updateHomedAll() { //check if all is homed
    ...
    static INLINE void setZHomed(uint8_t b) {
        ...
        static INLINE uint8_t isAHomed() {        //6axis Homing
        return flag4 & PRINTER_FLAG4_A_HOMED;
    }

    static INLINE void setAHomed(uint8_t b) {
        flag4 = (b ? flag4 | PRINTER_FLAG4_A_HOMED : flag4 & ~PRINTER_FLAG4_A_HOMED);
        updateHomedAll();
    }

        static INLINE uint8_t isBHomed() {        //6axis Homing
        return flag4 & PRINTER_FLAG4_B_HOMED;
    }

    static INLINE void setBHomed(uint8_t b) {
        flag4 = (b ? flag4 | PRINTER_FLAG4_B_HOMED : flag4 & ~PRINTER_FLAG4_B_HOMED);
        updateHomedAll();
    }
        static INLINE uint8_t isCHomed() {        //6axis Homing
        return flag4 & PRINTER_FLAG4_C_HOMED;
    }

    static INLINE void setCHomed(uint8_t b) {
        flag4 = (b ? flag4 | PRINTER_FLAG4_C_HOMED : flag4 & ~PRINTER_FLAG4_C_HOMED);
        updateHomedAll();
    }
#endif
    ...
    static INLINE void startXStep() {
        ...
        WRITE(X_STEP_PIN, START_STEP_WITH_HIGH);//test 6axis
#if FEATURE_TWO_XSTEPPER
        WRITE(X2_STEP_PIN, START_STEP_WITH_HIGH);
#endif
#endif
    ...
    static INLINE void startBStep()
    {
#if  NUM_AXES>=5
        WRITE(B_STEP_PIN, START_STEP_WITH_HIGH);
#endif
    }
    static INLINE void startCStep()
    {
#if  NUM_AXES>=6
        WRITE(C_STEP_PIN, START_STEP_WITH_HIGH);
#endif
    }

    static INLINE void endXYZSteps() ///All 6axis included
    {
        WRITE(X_STEP_PIN, !START_STEP_WITH_HIGH);
#if FEATURE_TWO_XSTEPPER || DUAL_X_AXIS
        WRITE(X2_STEP_PIN, !START_STEP_WITH_HIGH);
    ...
    static INLINE void disableAllowedStepper() {
        ...
        if(DISABLE_B) disableBStepper();
        if(DISABLE_C) disableCStepper();
#endif
    }
    ...
    static INLINE float realBPosition() {
        return currentPosition[B_AXIS];
    }
    static INLINE float realCPosition() {
        return currentPosition[C_AXIS];
    }
    /** \brief copies currentPosition to parameter. */  
    //6axis need updates??? use mostly for reading
    ...
    */
    ...
    static uint8_t moveTo(float x, float y, float z, float a, float b, float c, float e, float f); //6axis
    ...
    */
    ...
    static uint8_t moveToReal(float x, float y, float z, float a, float b, float c,  float e, float f, bool pathOptimize = true);
    ...
    static void setOrigin(float xOff, float yOff, float zOff, float aOff, float bOff, float cOff); //6axis
    ...
    static bool isPositionAllowed(float x, float y, float z, float a, float b, float c);
    ...
    static void GoToMemoryPosition(bool x, bool y, bool z, bool a, bool b, bool c, bool e, float feed); //6axis
    ...
    static void homeZAxis();  //6axis home abc???
