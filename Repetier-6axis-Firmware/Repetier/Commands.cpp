...
#endif
    for(int i = 0; i <= 4; i++) microstepMode(i, microstep_modes[i]); //6axis
#endif
}
#endif
/**
...
void Commands::processGCode(GCode *com) {
...
#endif // defined
        if(com->hasS()) Printer::setNoDestinationCheck(com->S != 0);  //6axis compute G0 and G1
        if(Printer::setDestinationStepsFromGCode(com)) // For X Y Z A B C E F 6axis
#if NONLINEAR_SYSTEM
    ...
    case 92: { // G92
        ...
        float aOff = Printer::coordinateOffset[A_AXIS];
        float bOff = Printer::coordinateOffset[B_AXIS];
        float cOff = Printer::coordinateOffset[C_AXIS];
        ...
        if(com->hasA()) aOff = Printer::convertToMM(com->A) - Printer::currentPosition[A_AXIS];
        if(com->hasB()) bOff = Printer::convertToMM(com->B) - Printer::currentPosition[B_AXIS];
        if(com->hasC()) cOff = Printer::convertToMM(com->C) - Printer::currentPosition[C_AXIS];
        
        Printer::setOrigin(xOff, yOff, zOff, aOff, bOff, cOff);
        if(com->hasE()) {
		...
		if(com->hasX() || com->hasY() || com->hasZ() || com->hasA() || com->hasB() || com->hasC()) {
            ...
            Com::printF(PSTR("A_OFFSET:"), Printer::coordinateOffset[A_AXIS], 3);
			Com::printF(PSTR(" B_OFFSET:"), Printer::coordinateOffset[B_AXIS], 3);
			Com::printFLN(PSTR(" C_OFFSET:"), Printer::coordinateOffset[C_AXIS], 3);
		}
    }
    break;
#if DRIVE_SYSTEM == DELTA
