...
typedef struct {
    ...
    flag8_t dirABC;  
#if NUM_AXES <=3
    uint16_t deltaSteps[TOWER_ARRAY];              ///< Number of steps in move. originally TOWER_ARRAY
#else
    uint16_t deltaSteps[Z_AXIS_ARRAY];
#endif
    ...
    #if NUM_AXES>=4
        return (dir & XYZE_STEP) == 0 && (dirABC & ABC_STEP) == 0; //6axis
    #else
  ...
  //6axis................................
      inline void setAMoveFinished() {
        dirABC &= ~ASTEP;
    }
     inline void setBMoveFinished() {
        dirABC &= ~BSTEP;
    }
     inline void setCMoveFinished() {
        dirABC &= ~CSTEP;
    }
    inline bool isAMove() {
        return (dirABC & ASTEP);
    ...
    inline bool isABCMove() {
        return dirABC & ABC_STEP;
    }
    inline bool isAPositiveMove() {
        return (dirABC & A_STEP_DIRPOS) == A_STEP_DIRPOS;
    }
    inline bool isANegativeMove() {
        return (dirABC & A_STEP_DIRPOS) == ASTEP;
    }
    inline bool isBPositiveMove() {
        return (dirABC & B_STEP_DIRPOS) == B_STEP_DIRPOS;
    }
    inline bool isBNegativeMove() {
        return (dirABC & B_STEP_DIRPOS) == BSTEP;
    }
    inline bool isCPositiveMove() {
        return (dirABC & C_STEP_DIRPOS) == C_STEP_DIRPOS;
    }
    inline bool isCNegativeMove() {
        return (dirABC & C_STEP_DIRPOS) == CSTEP;
    }
    inline bool isMoveOfABCAxis(uint8_t axis) {///axis A=0, B=1, C=2
        return (dirABC & (ASTEP << axis));
    }
     inline void setMoveOfABCAxis(uint8_t axis) {///axis A=0, B=1, C=2
        dirABC |= ASTEP << axis;
    }
    inline void setPositiveMoveOfABCAxis(uint8_t axis) {///axis A=0, B=1, C=2
        dirABC |= A_STEP_DIRPOS << axis;
    }
    inline void setPositiveDirectionForABCAxis(uint8_t axis) {///axis A=0, B=1, C=2
        dirABC |= A_DIRPOS << axis;
    }
...
public:
    ...
    INLINE void startBStep() {
        Printer::startBStep();
#ifdef DEBUG_STEPCOUNT
        totalStepsRemaining--;
#endif
    }
    INLINE void startCStep() {
        Printer::startCStep();
#ifdef DEBUG_STEPCOUNT
        totalStepsRemaining--;
#endif
    ...
    static void moveRelativeDistanceInSteps(int32_t x, int32_t y, int32_t z, int32_t a,int32_t b,int32_t c, int32_t e, float feedrate, bool waitEnd, bool check_endstop, bool pathOptimize = true);
    ...
    static void moveRelativeDistanceInStepsReal(int32_t x, int32_t y, int32_t z, int32_t a,int32_t b,int32_t c,int32_t e, float feedrate, bool waitEnd, bool pathOptimize = true);
    ...
    static inline void calculateDirectionAndDelta(int32_t difference[], ufast8_t *dir,  ufast8_t *dirABC, int32_t delta[]);
    static inline uint8_t calculateDistance(float axis_diff[], uint8_t dir, float *distance);
    static inline uint8_t calculateDistance(float axis_diff[], uint8_t dir,  ufast8_t dirABC,  float *distance);
