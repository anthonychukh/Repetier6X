...
float Printer::axisStepsPerMM[E_AXIS_ARRAY] = {XAXIS_STEPS_PER_MM, YAXIS_STEPS_PER_MM, ZAXIS_STEPS_PER_MM, AAXIS_STEPS_PER_MM, BAXIS_STEPS_PER_MM, CAXIS_STEPS_PER_MM, 1}; ///< Number of steps per mm needed.
float Printer::invAxisStepsPerMM[E_AXIS_ARRAY]; ///< Inverse of axisStepsPerMM for faster conversion
float Printer::maxFeedrate[E_AXIS_ARRAY] = {MAX_FEEDRATE_X, MAX_FEEDRATE_Y, MAX_FEEDRATE_Z, MAX_FEEDRATE_A, MAX_FEEDRATE_B, MAX_FEEDRATE_C }; ///< Maximum allowed feedrate.
float Printer::homingFeedrate[Z_AXIS_ARRAY] = {HOMING_FEEDRATE_X, HOMING_FEEDRATE_Y, HOMING_FEEDRATE_Z, HOMING_FEEDRATE_A, HOMING_FEEDRATE_B, HOMING_FEEDRATE_C};
...
float Printer::maxAccelerationMMPerSquareSecond[E_AXIS_ARRAY] = {MAX_ACCELERATION_UNITS_PER_SQ_SECOND_X, MAX_ACCELERATION_UNITS_PER_SQ_SECOND_Y, MAX_ACCELERATION_UNITS_PER_SQ_SECOND_Z, MAX_ACCELERATION_UNITS_PER_SQ_SECOND_A, MAX_ACCELERATION_UNITS_PER_SQ_SECOND_B, MAX_ACCELERATION_UNITS_PER_SQ_SECOND_C}; ///< X, Y, Z and E max acceleration in mm/s^2 for printing moves or retracts. 6axis add abc
float Printer::maxTravelAccelerationMMPerSquareSecond[E_AXIS_ARRAY] = {MAX_TRAVEL_ACCELERATION_UNITS_PER_SQ_SECOND_X, MAX_TRAVEL_ACCELERATION_UNITS_PER_SQ_SECOND_Y, MAX_TRAVEL_ACCELERATION_UNITS_PER_SQ_SECOND_Z, MAX_TRAVEL_ACCELERATION_UNITS_PER_SQ_SECOND_A, MAX_TRAVEL_ACCELERATION_UNITS_PER_SQ_SECOND_B, MAX_TRAVEL_ACCELERATION_UNITS_PER_SQ_SECOND_C}; ///< X, Y, Z max acceleration in mm/s^2 for travel moves
...
#if NONLINEAR_SYSTEM
#if NUM_AXES <=3
long Printer::currentNonlinearPositionSteps[E_TOWER_ARRAY];//nonlinear move delta step
#else
long Printer::currentNonlinearPositionSteps[E_AXIS_ARRAY];//nonlinear move delta step
#endif
...
float Printer::coordinateOffset[Z_AXIS_ARRAY] = {0, 0, 0, 0, 0, 0}; //6axis added 3 more values for abc
uint8_t Printer::flag0 = 0;
uint8_t Printer::flag1 = 0;
uint8_t Printer::flag2 = 0;
uint8_t Printer::flag3 = 0;      //check for homing XYZ, door
uint8_t Printer::flag4 = 0;      //6axis check for homing ABC
...
#else
int32_t Printer::realDeltaPositionSteps[Z_AXIS_ARRAY];
#endif
...
#endif
#if NUM_AXES >=4 //6axis software endstop
int32_t Printer::aMinStepsAdj, Printer::bMinStepsAdj, Printer::cMinStepsAdj; // adjusted to cover extruder/probe offsets
int32_t Printer::aMaxStepsAdj, Printer::bMaxStepsAdj, Printer::cMaxStepsAdj;
...
int32_t Printer::aMaxSteps;                   ///< For software endstops, limit of move in positive direction. 6axis
int32_t Printer::bMaxSteps;                   ///< For software endstops, limit of move in positive direction.
int32_t Printer::cMaxSteps;                   ///< For software endstops, limit of move in positive direction.
int32_t Printer::aMinSteps;                   ///< For software endstops, limit of move in negative direction.
int32_t Printer::bMinSteps;                   ///< For software endstops, limit of move in negative direction.
int32_t Printer::cMinSteps;                   ///< For software endstops, limit of move in negative direction.
...
float Printer::zMin;
float Printer::aLength;       //6axis
float Printer::aMin;
float Printer::bLength;
float Printer::bMin;
float Printer::cLength;
float Printer::cMin;
...
float Printer::backlashA;
float Printer::backlashB;
float Printer::backlashC;
...
float Printer::memoryA = IGNORE_COORDINATE;
float Printer::memoryB = IGNORE_COORDINATE;
float Printer::memoryC = IGNORE_COORDINATE;
...
// #if NUM_AXES >= 4                                         //6axis copy from above !nonlinear
// void Printer::constrainDestinationCoords() {
//     if(isNoDestinationCheck() || isHoming()) return;

// #if min_software_endstop_a
//     if (destinationSteps[A_AXIS] < aMinStepsAdj) destinationSteps[A_AXIS] = aMinStepsAdj;
// #endif
// #if min_software_endstop_b
//     if (destinationSteps[B_AXIS] < bMinStepsAdj) destinationSteps[B_AXIS] = bMinStepsAdj;
// #endif
// #if min_software_endstop_c
//     if (destinationSteps[C_AXIS] < cMinStepsAdj) destinationSteps[C_AXIS] = cMinStepsAdj;
// #endif

// #if max_software_endstop_a
//     if (destinationSteps[A_AXIS] > aMaxStepsAdj) destinationSteps[A_AXIS] = aMaxStepsAdj;
// #endif
// #if max_software_endstop_b
//     if (destinationSteps[B_AXIS] > bMaxStepsAdj) destinationSteps[B_AXIS] = bMaxStepsAdj;
// #endif
// #if max_software_endstop_c
//     if (destinationSteps[C_AXIS] > cMaxStepsAdj) destinationSteps[C_AXIS] = cMaxStepsAdj;
// #endif
...
bool Printer::isPositionAllowed(float x, float y, float z, float a, float b, float c) {
    if(isNoDestinationCheck()) return true;
    bool allowed = true;
#if DRIVE_SYSTEM == DELTA
    if(!isHoming()) {
...
#endif
// #if NUM_AXES >= 4        //6axis isPositionAllowed
//     allowed = allowed && (a >= 0) && (a <= aLength + 0.05 + ENDSTOP_A_BACK_ON_HOME);
//     allowed = allowed && (b >= 0) && (b <= aLength + 0.05 + ENDSTOP_B_BACK_ON_HOME);
//     allowed = allowed && (c >= 0) && (c <= aLength + 0.05 + ENDSTOP_C_BACK_ON_HOME);
// #endif
    /*#if DUAL_X_AXIS
        // Prevent carriage hit by disallowing moves inside other parking direction.
        if(Extruder::current->id == 0) {
...
//6axis not related. only for delta axis ie xyz
//could use similar approach for ABC SPM system
...
void Printer::updateDerivedParameter() {  
...
#endif
#if DRIVE_SYSTEM == DELTA   //6axis This calculate delta movement
    ...
    // 6axis consider acceleration for ABC axes???
...
#else  //6axis
  if(Printer::isHomedAll()) { // for safety move only when homed!
	moveToReal(EEPROM::parkX(),EEPROM::parkY(),IGNORE_COORDINATE, EEPROM::parkA(), EEPROM::parkB(), EEPROM::parkC(), IGNORE_COORDINATE, Printer::maxFeedrate[X_AXIS], true);
	moveToReal(IGNORE_COORDINATE,IGNORE_COORDINATE,RMath::min(zMin + zLength, currentPosition[Z_AXIS] + EEPROM::parkZ()),IGNORE_COORDINATE, Printer::maxFeedrate[Z_AXIS], true);
  }
#endif
...
uint8_t Printer::moveTo(float x, float y, float z, float e, float f) {
    ...
    Com::printFLN(PSTR("moveTo 3 axis"));
#endif
...
uint8_t Printer::moveTo(float x, float y, float z, float a, float b, float c, float e, float f) {
    #ifdef DEBUG_STEPCOUNT
    Com::printFLN(PSTR("moveTo 6 axis"));
#endif
    ...
    if(b != IGNORE_COORDINATE)
        destinationSteps[B_AXIS] = (b) * axisStepsPerMM[B_AXIS];
    if(c != IGNORE_COORDINATE)
        destinationSteps[C_AXIS] = (c) * axisStepsPerMM[C_AXIS];
    if(e != IGNORE_COORDINATE)
        destinationSteps[E_AXIS] = e * axisStepsPerMM[E_AXIS];
	else
		destinationSteps[E_AXIS] = currentPositionSteps[E_AXIS];
...
#ifdef DEBUG_STEPCOUNT
    Com::printFLN(PSTR("moveToReal 3 axis"));
#endif
...
//6axis moveToReal. End with send to queueNonlinearMove
uint8_t Printer::moveToReal(float x, float y, float z, float a, float b, float c, float e, float f, bool pathOptimize) {  
    #ifdef DEBUG_STEPCOUNT
    Com::printFLN(PSTR("moveToReal 6 axis"));
#endif
    ...
    if(b == IGNORE_COORDINATE)
        b = currentPosition[B_AXIS];
    else
        currentPosition[B_AXIS] = b;
    if(c == IGNORE_COORDINATE)
        c = currentPosition[C_AXIS];
    else
        currentPosition[C_AXIS] = c;

    transformToPrinter(x + Printer::offsetX, y + Printer::offsetY, z + Printer::offsetZ, x, y, z);
    z += offsetZ2;

    // a = lastCmdPos[A_AXIS];  //replicate last option in transformToPrinter
    // b = lastCmdPos[B_AXIS];
    // c = lastCmdPos[C_AXIS];
    ...
    destinationSteps[B_AXIS] = static_cast<int32_t>(floor(b * axisStepsPerMM[B_AXIS] + 0.5f));
    destinationSteps[C_AXIS] = static_cast<int32_t>(floor(c * axisStepsPerMM[C_AXIS] + 0.5f));
    if(e != IGNORE_COORDINATE && !Printer::debugDryrun()
#if MIN_EXTRUDER_TEMP > 30
            && (Extruder::current->tempControl.currentTemperatureC > MIN_EXTRUDER_TEMP || Printer::isColdExtrusionAllowed() || Extruder::current->tempControl.sensorType == 0)
#endif
...
void Printer::setOrigin(float xOff, float yOff, float zOff, float aOff, float bOff, float cOff) {
    coordinateOffset[X_AXIS] = xOff;// offset from G92
    coordinateOffset[Y_AXIS] = yOff;
    coordinateOffset[Z_AXIS] = zOff;

    coordinateOffset[A_AXIS] = aOff;  //6axis limit to 6axis only??? num_axes
    coordinateOffset[B_AXIS] = bOff;
    coordinateOffset[C_AXIS] = cOff;
}
...
void Printer::updateCurrentPosition(bool copyLastCmd) {
...
#ifdef DEBUG_STEPCOUNT
            Com::printFLN(PSTR("now updateCurrentPosition"));
#endif
...
#endif
#if NUM_AXES >=5
    currentPosition[B_AXIS] = static_cast<float>(currentPositionSteps[B_AXIS]) * invAxisStepsPerMM[B_AXIS];
#endif
#if NUM_AXES >=6
    currentPosition[C_AXIS] = static_cast<float>(currentPositionSteps[C_AXIS]) * invAxisStepsPerMM[C_AXIS];
#endif
    ...
    if(copyLastCmd) {
        ...
        lastCmdPos[B_AXIS] = currentPosition[B_AXIS];
        lastCmdPos[C_AXIS] = currentPosition[C_AXIS];
#endif
    }
}
...
void Printer::updateCurrentPositionSteps() {
#ifdef DEBUG_STEPCOUNT
            Com::printFLN(PSTR(" updateCurrentPositionStep"));
#endif
    float x_rotc, y_rotc, z_rotc, a_rotc, b_rotc, c_rotc;
    ...
    a_rotc = currentPosition[A_AXIS];  //replicate last option in transformToPrinter
    b_rotc = currentPosition[B_AXIS];
    c_rotc = currentPosition[C_AXIS];
...
#if NUM_AXES >= 4
    currentPositionSteps[A_AXIS] = static_cast<int32_t>(floor(a_rotc * axisStepsPerMM[A_AXIS] + 0.5f));
// #endif
// #if NUM_AXES >= 5
    currentPositionSteps[B_AXIS] = static_cast<int32_t>(floor(b_rotc * axisStepsPerMM[B_AXIS] + 0.5f));
// #endif
// #if NUM_AXES >= 6
    currentPositionSteps[C_AXIS] = static_cast<int32_t>(floor(c_rotc * axisStepsPerMM[C_AXIS] + 0.5f));
#endif
#if NONLINEAR_SYSTEM
...
uint8_t Printer::setDestinationStepsFromGCode(GCode *com) {  //6axis move according to G0/G1 input, reference in Command.cpp
    register int32_t p;
    float x, y, z, a, b, c;
    bool posAllowed = true;
#if FEATURE_RETRACTION
    if(com->hasNoXYZ() && com->hasNoABC() && com->hasE() && isAutoretract()) { // convert into auto retract
    ...
    if(!(com->hasNoXYZ() && com->hasNoABC())) {
#endif                         //6axis set all position regardless of drive system
        ...
        //#if NUM_AXES >=4
            if(com->hasA()) {
                lastCmdPos[A_AXIS] = currentPosition[A_AXIS] = convertToMM(com->A) - coordinateOffset[A_AXIS]; //6axis
#ifdef DEBUG_STEPCOUNT     
                Com::printF(PSTR("hasA"), 00);  
                Com::printF(PSTR(" a position:"), currentPosition[A_AXIS]);  
#endif         
            }
        //#endif
        //#if NUM_AXES >=5
            if(com->hasB()){
                lastCmdPos[B_AXIS] = currentPosition[B_AXIS] = convertToMM(com->B) - coordinateOffset[B_AXIS];
#ifdef DEBUG_STEPCOUNT  
                Com::printF(PSTR(" b position:"), currentPosition[B_AXIS]); 
#endif
            }
        //#endif
        //#if NUM_AXES >=6
            if(com->hasC()) {
                lastCmdPos[C_AXIS] = currentPosition[C_AXIS] = convertToMM(com->C) - coordinateOffset[C_AXIS];
#ifdef DEBUG_STEPCOUNT  
                Com::printF(PSTR(" c position:"), currentPosition[C_AXIS]); 
#endif
            }
        //#endif
        } else {
        ...
        //#if NUM_AXES >=4
            if(com->hasA()) { 
                currentPosition[A_AXIS] = (lastCmdPos[A_AXIS] += convertToMM(com->A));  //6axis
#ifdef DEBUG_STEPCOUNT  
                Com::printF(PSTR("hasA")); 
                Com::printF(PSTR(" a position:"), currentPosition[A_AXIS]); 
#endif
            }
        //#endif
        //#if NUM_AXES >=5
            if(com->hasB()){
                 currentPosition[B_AXIS] = (lastCmdPos[B_AXIS] += convertToMM(com->B));
#ifdef DEBUG_STEPCOUNT  
                Com::printF(PSTR("hasB")); 
                Com::printF(PSTR(" b position:"), currentPosition[B_AXIS]); 
#endif
            }
        //#endif
        //#if NUM_AXES >=6
            if(com->hasC()) {
                currentPosition[C_AXIS] = (lastCmdPos[C_AXIS] += convertToMM(com->C));
#ifdef DEBUG_STEPCOUNT  
                Com::printF(PSTR("hasC")); 
                Com::printF(PSTR(" c position:"), currentPosition[C_AXIS]); 
#endif
            }
        //#endif
        ...
        destinationSteps[Z_AXIS] = static_cast<int32_t>(floor(z * axisStepsPerMM[Z_AXIS] + 0.5f));
        //#if NUM_AXES >=4
        destinationSteps[A_AXIS] = static_cast<int32_t>(floor(a * axisStepsPerMM[A_AXIS] + 0.5f)); // 6axis
        //#endif
        //#if NUM_AXES >=5
        destinationSteps[B_AXIS] = static_cast<int32_t>(floor(b * axisStepsPerMM[B_AXIS] + 0.5f));
        //#endif
        ...
        //#endif
#ifdef DEBUG_STEPCOUNT
         Com::printF(PSTR("setDestinationStepsFromGCode:: x DesStep:"), destinationSteps[X_AXIS]);
         Com::printF(PSTR(" y DesStep:"), destinationSteps[Y_AXIS]);
         Com::printF(PSTR(" z DesStep:"), destinationSteps[Z_AXIS]);
         Com::printF(PSTR(" a DesStep:"), destinationSteps[A_AXIS]);
         Com::printF(PSTR(" b DesStep:"), destinationSteps[B_AXIS]);
         Com::printFLN(PSTR(" c DesStep:"), destinationSteps[C_AXIS]);
#endif
...
#ifdef DEBUG_STEPCOUNT
        if(!posAllowed){
            Com::printF(PSTR(" position NOT allowed ")); 
        }
#endif
#if DISTORTION_CORRECTION == 0
    }
    ...
    if(!posAllowed) {
        ...
        Com::printF(PSTR(" move ignored ")); 
        return false; // ignore move
    }
    return !com->hasNoXYZ() || !com->hasNoABC() || (com->hasE() && destinationSteps[E_AXIS] != currentPositionSteps[E_AXIS]); // ignore unproductive moves
}
...
#if NUM_AXES >=4
    SET_OUTPUT(A_STEP_PIN); //6axis Set step pin
#endif
#if NUM_AXES >=5
...
#endif
#if A_DIR_PIN > -1 && NUM_AXES >=4          //6axis Set dir pin
    SET_OUTPUT(A_DIR_PIN);
#endif
#if B_DIR_PIN > -1 && NUM_AXES >=5
    SET_OUTPUT(B_DIR_PIN);
#endif
#if C_DIR_PIN > -1 && NUM_AXES >=6
...
#if A_ENABLE_PIN > -1 && NUM_AXES >=4          //6axis Set enable pin
    SET_OUTPUT(A_ENABLE_PIN);
    WRITE(A_ENABLE_PIN, !A_ENABLE_ON);
#endif
#if B_ENABLE_PIN > -1 && NUM_AXES >=5
    SET_OUTPUT(B_ENABLE_PIN);
    WRITE(B_ENABLE_PIN, !B_ENABLE_ON);
#endif
#if C_ENABLE_PIN > -1 && NUM_AXES >=6
    SET_OUTPUT(C_ENABLE_PIN);
    WRITE(C_ENABLE_PIN, !C_ENABLE_ON);
#endif
...
// Initialize jam sensors
    ...
    aLength = A_MAX_LENGTH;    //6axis
    bLength = B_MAX_LENGTH;
    cLength = C_MAX_LENGTH;
    xMin = X_MIN_POS;
    yMin = Y_MIN_POS;
    zMin = Z_MIN_POS;
    aMin = A_MIN_POS;    //6axis
    bMin = B_MIN_POS;
    cMin = C_MIN_POS;
#if DRIVE_SYSTEM == DELTA
    radius0 = ROD_RADIUS;
    ...
    #if NUM_AXES >= 4
    backlashA = A_BACKLASH;   //6axis
    backlashB = B_BACKLASH;
    backlashC = C_BACKLASH;
    #endif
...
#if NUM_AXES >= 4
    currentPosition[A_AXIS] = currentPosition[B_AXIS] = currentPosition[C_AXIS] =  0.0;   //6axis
#endif
    ...
    transformCartesianStepsToDeltaSteps(Printer::currentPositionSteps, Printer::currentNonlinearPositionSteps);
#ifdef DEBUG_STEPCOUNT
        Com::printF(PSTR("Setup:: nonLinearPosStepX: "), Printer::currentNonlinearPositionSteps[X_AXIS]);
        Com::printF(PSTR(" nonLinearPosStepY: "), Printer::currentNonlinearPositionSteps[Y_AXIS]);
        Com::printF(PSTR(" nonLinearPosStepZ: "), Printer::currentNonlinearPositionSteps[Z_AXIS]);
        Com::printF(PSTR(" nonLinearPosStepA: "), Printer::currentNonlinearPositionSteps[A_AXIS]);
        Com::printF(PSTR(" nonLinearPosStepB: "), Printer::currentNonlinearPositionSteps[B_AXIS]);
        Com::printFLN(PSTR(" nonLinearPosStepC: "), Printer::currentNonlinearPositionSteps[C_AXIS]);
#endif
...
void Printer::GoToMemoryPosition(bool x, bool y, bool z, bool a, bool b, bool c, bool e, float feed) {
    if(memoryF < 0) return; // Not stored before call, so we ignore it
    bool all = !(x || y || z);
    moveToReal((all || x ? (lastCmdPos[X_AXIS] = memoryX) : IGNORE_COORDINATE)
               , (all || y ? (lastCmdPos[Y_AXIS] = memoryY) : IGNORE_COORDINATE)
               , (all || z ? (lastCmdPos[Z_AXIS] = memoryZ) : IGNORE_COORDINATE)
               , (all || a ? (lastCmdPos[A_AXIS] = memoryA) : IGNORE_COORDINATE)
               , (all || b ? (lastCmdPos[B_AXIS] = memoryB) : IGNORE_COORDINATE)
               , (all || c ? (lastCmdPos[C_AXIS] = memoryC) : IGNORE_COORDINATE)
               , (e ? memoryE : IGNORE_COORDINATE),
               feed);
    feedrate = memoryF;
    updateCurrentPosition(false);
...
#if NUM_AXES >=4  //6axis
    long da = -aMinSteps;// - EEPROM::deltaTowerXOffsetSteps(); //need to enable offset in eeprom
    long db = -bMinSteps;// - EEPROM::deltaTowerYOffsetSteps();
    long dc = -cMinSteps;// - EEPROM::deltaTowerZOffsetSteps();
    long dm6 = RMath::min(da, db, dc);
    //Com::printFLN(Com::tTower1,dx);
    //Com::printFLN(Com::tTower2,dy);
    //Com::printFLN(Com::tTower3,dz);
    da -= dm6; // now all dxyz are positive
    db -= dm6;
    dc -= dm6;
    currentPositionSteps[A_AXIS] = 0; // here we should be
    currentPositionSteps[B_AXIS] = 0;
    currentPositionSteps[C_AXIS] = 0;
#endif
    transformCartesianStepsToDeltaSteps(currentPositionSteps, currentNonlinearPositionSteps);
...
#if NUM_AXES >=4  //6axis
    currentNonlinearPositionSteps[A_AXIS] -= da;
    currentNonlinearPositionSteps[B_AXIS] -= db;
    currentNonlinearPositionSteps[C_AXIS] -= dc;
    // currentNonlinearPositionSteps[A_AXIS] = 0;
    // currentNonlinearPositionSteps[B_AXIS] = 0;
    // currentNonlinearPositionSteps[C_AXIS] = 0;
...
#if NUM_AXES >=4  //6axis
    currentPositionSteps[A_AXIS] = 0; // now we are really here
    currentPositionSteps[B_AXIS] = 0;
    currentPositionSteps[C_AXIS] = 0; // Extruder is now exactly in the delta center
    coordinateOffset[A_AXIS] = 0;
    coordinateOffset[B_AXIS] = 0;
    coordinateOffset[C_AXIS] = 0;
...
#if NUM_AXES >=4  //6axis
    realDeltaPositionSteps[A_AXIS] = currentNonlinearPositionSteps[A_AXIS];
    realDeltaPositionSteps[B_AXIS] = currentNonlinearPositionSteps[B_AXIS];
    realDeltaPositionSteps[C_AXIS] = currentNonlinearPositionSteps[C_AXIS];
