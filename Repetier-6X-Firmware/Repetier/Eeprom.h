...
#define EPR_PARK_A		              1068
#define EPR_PARK_B                            1072
#define EPR_PARK_C                            1076
...
static inline void setTowerZFloor(float newZ) {
    ...
    static inline float parkA() {
	    #if EEPROM_MODE != 0
	    return HAL::eprGetFloat(EPR_PARK_A);
	    #else
	    return PARK_POSITION_Z_RAISE;
	    #endif
    }
    static inline float parkB() {
	    #if EEPROM_MODE != 0
	    return HAL::eprGetFloat(EPR_PARK_B);
	    #else
	    return PARK_POSITION_Z_RAISE;
	    #endif
    }
    static inline float parkC() {
	    #if EEPROM_MODE != 0
	    return HAL::eprGetFloat(EPR_PARK_C);
	    #else
	    return PARK_POSITION_Z_RAISE;
	    #endif
    }
